...
export default function ReviewsPage() {
  ...
  const handleEditReview = (review: Review) => {
    ...
    const project = MOCK_PROJECTS.find(p => p.id === review.projectId) || {
      id: review.projectId, name: review.projectName, category: "", description: "", rating: 0, reviews: 0,
    };
    setSelectedProject(project);
  };
  ...
  const handleSubmit = (data: { rating: number; comment: string }) => {
    ...
    if (editingReview) {
      ...
      reviewService.addReview({ projectId: selectedProject.id, projectName: selectedProject.name, userAddress: publicKey, ...data });
    }
    setReviews(reviewService.getReviews());
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <div>
            ...
            <p className="text-zinc-500 dark:text-zinc-400">Transparent feedback from the Stellar ecosystem.</p>
          </div>
          {!isAddingReview && !editingReview && (
            <div className="flex flex-wrap gap-2">
              {MOCK_PROJECTS.map(p => (
                <button key={p.id} onClick={() => handleAddReview(p)}
        ...
        {(isAddingReview || editingReview) && selectedProject && (
          <div className="mb-12">
            <ReviewForm
              ...
              onCancel={() => { setIsAddingReview(false); setEditingReview(null); setSelectedProject(null); }}
            />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-12">
          <section>
            ...
            <ReviewList reviews={reviews} currentUserAddress={publicKey} onEdit={handleEditReview} onDelete={handleDeleteReview} />
          </section>
        </div>
      </div>
    </div>
